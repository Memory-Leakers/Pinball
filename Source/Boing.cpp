...
Boing::Boing(std::string name,std::string tag, Application* _app, int x, int y, int radius) : GameObject(name, tag, _app)
{
    ...
    renderObjects[0].scale = (1.0f / 60.0f) * radius;
	...
	// Create PhysBody
	pBody = _app->physics->CreateCircle(x, y, radius, this);
...
{
	if (col->gameObject && col->gameObject->CompareTag("Player") && col->gameObject->layer == layer)
	{
		counter = 0.1f * FPS;
